...
# -*- coding: utf-8 -*-
"""
Data Isolation Test Suite (Simple ASCII Version)
...
def test_user_login(email):
    ...
    print_test("Login successful", passed, f"Token: {session['session_token'][:20]}..." if session else "")
...
def run_all_tests():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"ERROR: Backend not responding correctly (status {response.status_code})")
            return False
    except Exception as e:
    ...
    if not (user1_session and user2_session):
        print(f"\nERROR: Could not establish sessions for testing")
        return False
