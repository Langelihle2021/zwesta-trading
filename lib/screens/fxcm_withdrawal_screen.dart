...
class FxcmWithdrawalScreen extends StatefulWidget {
  ...
  @override
  State<FxcmWithdrawalScreen> createState() => _FxcmWithdrawalScreenState();
}

class _FxcmWithdrawalScreenState extends State<FxcmWithdrawalScreen> {
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      final data = await FxcmTradingService.getWithdrawalNotifications(_userId);
  ...
  Future<void> _runProfitCheck() async {
    ...
    final result = await FxcmTradingService.profitCheck(
      targetProfit: target,
      userId: _userId,
    ...
    if (result['success'] == true && result['target_reached'] == true) {
      await FxcmTradingService.createWithdrawalNotification(
        userId: _userId,
      ...
      NotificationService.showNotification(
        title: 'FXCM Profit Target Reached!',
        body: 'P&L: \$${(result['current_pnl'] as num).toStringAsFixed(2)} — '
            'Positions closed. Withdraw on FXCM now.',
  ...
  Widget _buildInfoBanner() {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.info_outline, color: Color(0xFFCE93D8), size: 28),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('How FXCM Withdrawals Work',
                    style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14)),
                const SizedBox(height: 4),
                Text(
                  '1. Set a profit target\n'
                  '2. System monitors & auto-closes positions when target is hit\n'
                  '3. You get notified to withdraw on FXCM\'s website/app\n'
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('When open FXCM positions reach the target, they are auto-closed.',
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text('Now go to fxcm.com or the FXCM app to withdraw your funds.',
  ...
  Widget _buildNotificationsSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text('Profits realized — withdraw from FXCM when ready.',
            style: GoogleFonts.poppins(color: Colors.white38, fontSize: 12)),
        const SizedBox(height: 12),
        if (_loadingNotifs)
