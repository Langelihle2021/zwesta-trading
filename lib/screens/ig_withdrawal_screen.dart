...
class IGWithdrawalScreen extends StatefulWidget {
  const IGWithdrawalScreen({Key? key}) : super(key: key);

  @override
  State<IGWithdrawalScreen> createState() => _IGWithdrawalScreenState();
}

class _IGWithdrawalScreenState extends State<IGWithdrawalScreen> {
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      final data = await IGTradingService.getWithdrawalNotifications(_userId);
  ...
  Future<void> _runProfitCheck() async {
    ...
    final result = await IGTradingService.profitCheck(
      targetProfit: target,
      userId: _userId,
    ...
    if (result['success'] == true && result['target_reached'] == true) {
      // Create a withdrawal notification record
      await IGTradingService.createWithdrawalNotification(
        userId: _userId,
      ...
      NotificationService.showNotification(
        title: '💰 IG Profit Target Reached!',
        body: 'P&L: \$${(result['current_pnl'] as num).toStringAsFixed(2)} — '
            'Positions closed. Withdraw on IG now.',
  ...
  Future<void> _markDone(String notifId) async {
    final res = await IGTradingService.markWithdrawalDone(notifId);
    if (res['success'] == true) {
      _showSnack('Marked as withdrawn');
      _loadNotifications();
    } else {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('How IG Withdrawals Work',
                    style: GoogleFonts.poppins(
                        color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14)),
                const SizedBox(height: 4),
                Text(
                  '1. Set a profit target\n'
                  '2. System monitors & auto-closes positions when target is hit\n'
                  '3. You get notified to withdraw on IG\'s website/app\n'
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('When your open IG positions reach the target, they are auto-closed.',
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(
                _checking ? 'Checking...' : 'Check & Auto-Close',
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Now go to ig.com or the IG app to withdraw your funds.',
                      style: GoogleFonts.poppins(
                          color: Colors.orangeAccent, fontSize: 12, fontWeight: FontWeight.w500),
                    ),
                  ),
                ],
              ),
    ...
    return _resultBox(
      ...
      extra: Column(
        ...
        children: [
          ...
          if (r['positions'] != null)
            ...List<Widget>.from((r['positions'] as List).take(5).map((p) {
              final pnl = p['pnl'] as num;
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.poppins(
                        color: pnl >= 0 ? const Color(0xFF69F0AE) : Colors.redAccent,
                        fontSize: 12,
  ...
  Widget _buildNotificationsSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text('Profits realized — withdraw from IG when ready.',
            style: GoogleFonts.poppins(color: Colors.white38, fontSize: 12)),
        const SizedBox(height: 12),
        if (_loadingNotifs)
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isPending
            ...
            : const Color(0xFF69F0AE).withOpacity(0.06),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isPending
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  isPending ? 'Withdrawal Ready' : 'Withdrawn',
                  style: GoogleFonts.poppins(
                    color: isPending ? Colors.orangeAccent : const Color(0xFF69F0AE),
                    fontSize: 14,
                    fontWeight: FontWeight.w600,
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    onPressed: () {
                      _showSnack('Open ig.com in your browser to withdraw');
                    },
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () => _markDone(n['notification_id']),
                  ),
                ),
              ],
            ),
